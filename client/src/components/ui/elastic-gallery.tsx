...
function ElasticGallery() {
  const items: ElasticItemProps[] = [
    ...
    },
    {
      ...
      src: "https://images.unsplash.com/photo-1517059224940-d4af9eec41b7?auto=format&fit=crop&w=600&q=80",
      ...
      <div className="mx-auto flex h-[500px] w-full max-w-6xl flex-col gap-2 px-4 md:h-[600px] md:flex-row md:gap-4">
        {items.map((item) => (
          <div
            ...
            className={cn(
              "relative cursor-pointer overflow-hidden rounded-2xl border border-neutral-800 bg-black/40 backdrop-blur-sm",
            ...
            <div className="absolute bottom-0 left-0 right-0 flex h-full flex-col justify-end p-4 md:p-8">
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <span className="rounded-full border border-red-500/30 bg-red-500/10 px-2 py-1 text-[10px] font-medium uppercase tracking-wider text-red-400 backdrop-blur-md md:px-3 md:text-xs">
                    {item.category}
                  </span>
                </div>
              ...
              >
                ...
                <span className="hidden whitespace-nowrap text-xl font-bold uppercase tracking-widest text-white/80 [writing-mode:vertical-rl] md:block">
                  {item.title}
                </span>

                {/* Mobile: Horizontal ID/Label */}
                <span className="block text-xs font-bold text-white/80 md:hidden">
                  {item.id}
                </span>
              </div>
            </div>
